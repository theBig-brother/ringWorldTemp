package io.github.lv.ecs.pawn.system.render
...
@Component
class PawnRenderSystem() : EntitySystem() {
    // 新增一个标志位来控制是否绘制调试信息
    @Inject
    ...
    override fun update(deltaTime: Float) {
        val entities = engineContainer.pawnEngine.getEntitiesFor(Family.all(PawnAppearanceComponent::class.java).get())
        ...
        for (entity in entities) {
            ...
            // 如果启用调试模式，绘制调试信息
            if (debugMode) {
                drawDebugInfo(pawnAppearanceComponent.unitSprite, camera)
            }
            resources.batch.use{
                pawnAppearanceComponent.unitSprite.setOriginCenter()
                pawnAppearanceComponent.unitSprite.draw(it)
            }
